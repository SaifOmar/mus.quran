...
That's it — the plugin ships prebuilt static binaries (`prebuilt/linux-amd64`
and `prebuilt/linux-arm64`) that it finds inside its own folder, so no extra
step is required.
...
```sh
quickshell ipc call quran status
quickshell ipc call quran playSurah <reciter> <surah>
quickshell ipc call quran download <reciter> [surah]
quickshell ipc call quran cacheInfo
quickshell ipc call quran clearCache
```
